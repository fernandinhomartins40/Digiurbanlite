    ...
    <div
      className={cn(
        ...
        isActive
          ...
          : 'border-indigo-200 bg-white hover:border-indigo-300 hover:bg-indigo-50/70'
      )}
    >
      <Link
        href={primary.href}
        onClick={onNavigate}
        className="group flex items-center gap-2.5 rounded-md bg-indigo-600 px-3 py-2.5 text-white shadow-sm transition-colors hover:bg-indigo-700"
      >
        ...
        <span className="min-w-0 flex-1">
          ...
          <span className="block truncate text-[11px] font-medium leading-4 text-indigo-100">
            Mapa, agenda e chamados
          </span>
        </span>
        <ChevronRight className="h-4 w-4 shrink-0 text-indigo-100 transition-transform group-hover:translate-x-0.5" />
      </Link>
        ...
        <div className="mt-2 grid grid-cols-3 gap-1.5">
          {secondaryItems.map((item) => {
            ...
            return (
              ...
              >
                <item.icon className={cn('h-3.5 w-3.5', itemActive ? 'text-white' : 'text-indigo-600')} />
