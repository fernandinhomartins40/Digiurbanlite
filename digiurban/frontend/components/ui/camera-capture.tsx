...
export function CameraCapture({
  ...
  const startCamera = useCallback(async () => {
    ...
    try {
      ...
      if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        setError('Seu navegador não suporta acesso à câmera. Use um navegador atualizado.');
        setIsCameraActive(false);
        return;
      }

      // Preferências de câmera otimizadas para compatibilidade multi-dispositivo
      const facingMode = documentType === 'foto_perfil' ? 'user' : 'environment';
      ...
      try {
        // Primeira tentativa: constraints completas
        ...
        try {
          ...
          try {
            // Terceira tentativa: apenas vídeo genérico
            constraints = {
              video: true,
              audio: false
            };
            ...
            throw thirdError; // Se todas as tentativas falharem, lançar erro
          }
        }
      }
      ...
      if (videoRef.current) {
        ...
        // Garantir reprodução automática em iOS
      ...
      if (err.name === 'NotAllowedError' || err.name === 'PermissionDeniedError') {
        ...
        errorMessage = 'Acesso à câmera bloqueado por motivos de segurança. Use HTTPS.';
      }
  ...
  useEffect(() => {
    // Iniciar câmera apenas uma vez quando o componente montar
    startCamera();
