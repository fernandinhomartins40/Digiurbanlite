...
export default function EmailDomainsPage() {
  ...
  const generateDNSRecords = (domain: EmailDomain, hostname: string) => {
    ...
    // DMARC Record - Sempre mostrar (alinhado com backend que sempre verifica)
    const dmarcValue = domain.dmarcPolicy || `v=DMARC1; p=none; rua=mailto:dmarc@${domain.domainName}`;
    records.push({
      type: 'TXT',
                  ...
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <CardDescription>
                          Configure estes registros no seu provedor de DNS e clique em "Verificar Todos" para validar via consulta DNS real
                        </CardDescription>
                      </div>
                      <button
                      ...
                      >
                        ...
                        {verifying ? 'Consultando DNS...' : 'Verificar Todos'}
                      </button>
                    </div>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
                      <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
                        <h4 className="font-semibold text-sm text-green-900 mb-1 flex items-center gap-2">
                          <CheckCircle className="w-4 h-4" />
                          Verificação DNS Real
                        </h4>
                        <p className="text-xs text-green-800">
                          Este sistema realiza consultas DNS reais usando <code className="bg-green-100 px-1 py-0.5 rounded">dns.resolveMx()</code> e <code className="bg-green-100 px-1 py-0.5 rounded">dns.resolveTxt()</code>.
                          Os resultados mostram os registros DNS realmente configurados no seu domínio, não são simulados.
                        </p>
                      </div>
