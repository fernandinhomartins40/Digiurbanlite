...
export default function MeusDocumentosCidadaoPage() {
  ...
  const fetchDocuments = async () => {
    setLoading(true);
    try {
      const response = await fetch('/api/documents/my-external-documents-citizen', {
        headers: {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/documents/upload-external-citizen', {
        method: 'POST',
        body: formData,
      });
                    ...
                    <CardContent className="p-4">
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex items-start gap-3 flex-1 min-w-0">
                          <FileText className="w-10 h-10 text-purple-600 shrink-0 mt-1" />
