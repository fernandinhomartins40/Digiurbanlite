...
export default function EditarSala() {
  ...
  const loadSala = async () => {
    try {
      ...
      // Converter array de equipamentos para string separada por vírgula
      let equipamentosString = '';
      if (data.equipamentos && Array.isArray(data.equipamentos)) {
        equipamentosString = data.equipamentos.join(', ');
      } else if (data.equipamentos) {
        equipamentosString = String(data.equipamentos);
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(`Erro: ${error.error || 'Falha ao atualizar sala'}`);
      }
    } catch (error) {
        ...
        <div className="mb-6 flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold flex items-center gap-2">
              ...
              Editar Sala/Consultório
            </h1>
            <p className="text-gray-600">Atualizar informações da sala de atendimento ou consultório</p>
          </div>
        </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(value) => setFormData({ ...formData, ativa: value === 'true' })}
                    >
                      <SelectTrigger id="ativa">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                    ...
                    <p className="text-sm text-gray-500 mt-1">
                      Se a sala está disponível para uso
                    </p>
                  </div>
