...
export default function PolíticasparaMulheresSugestoesPage() {
  const router = useRouter();
  const { suggestions, isLoading } = useServiceSuggestions('politicas-mulheres');
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      ...
      <div className="mb-8">
        <Button
          variant="ghost"
          onClick={() => router.push('/admin/secretarias/politicas-mulheres')}
          className="mb-4"
        >
          <ArrowLeft className="h-4 w-4 mr-2" />
          Voltar para Políticas para Mulheres
        </Button>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-2">
              Explore todas as {suggestions.length} sugestões disponíveis para Políticas para Mulheres
            </p>
          </div>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredSuggestions.map((suggestion) => (
            <Card key={suggestion.id} className="border-pink-200 bg-pink-50/50 hover:shadow-lg transition-shadow">
              <CardHeader>
