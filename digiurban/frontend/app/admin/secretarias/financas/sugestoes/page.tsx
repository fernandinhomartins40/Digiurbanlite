...
export default function FinançasSugestoesPage() {
  const router = useRouter();
  const { suggestions, isLoading } = useServiceSuggestions('financas');
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      ...
      <div className="mb-8">
        <Button
          variant="ghost"
          onClick={() => router.push('/admin/secretarias/financas')}
          className="mb-4"
        >
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-2">
              Explore todas as {suggestions.length} sugestões disponíveis para Finanças
            </p>
          </div>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredSuggestions.map((suggestion) => (
            <Card key={suggestion.id} className="border-emerald-200 bg-emerald-50/50 hover:shadow-lg transition-shadow">
              <CardHeader>
