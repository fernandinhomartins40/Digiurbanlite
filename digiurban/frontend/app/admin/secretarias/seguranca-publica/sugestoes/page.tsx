...
export default function SegurançaPúblicaSugestoesPage() {
  const router = useRouter();
  const { suggestions, isLoading } = useServiceSuggestions('seguranca-publica');
  ...
  const buildServiceCreationUrl = (suggestion: any) => {
    const params = new URLSearchParams({
      departmentCode: 'seguranca-publica',
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      ...
      <div className="mb-8">
        <Button
          variant="ghost"
          onClick={() => router.push('/admin/secretarias/seguranca-publica')}
          className="mb-4"
        >
          <ArrowLeft className="h-4 w-4 mr-2" />
          Voltar para Segurança Pública
        </Button>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-2">
              Explore todas as {suggestions.length} sugestões disponíveis para Segurança Pública
            </p>
          </div>
        </div>
      </div>
