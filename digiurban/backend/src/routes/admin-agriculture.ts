...
import { AuthenticatedRequest, SuccessResponse, ErrorResponse, UserWithRelations } from '../types';
 ...
 */
router.get('/producers/stats', adminAuthMiddleware, requireMinRole(UserRole.USER), asyncHandler(async (req, res) => {
 ...
 */
router.get('/production/overview', adminAuthMiddleware, requireMinRole(UserRole.USER), async (req: LocalAuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    // Single tenant: tenantId removido
 ...
 */
router.get('/budget/summary', adminAuthMiddleware, requireMinRole(UserRole.USER), async (req: LocalAuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    // Single tenant: tenantId removido
 ...
 */
router.get('/services/stats', adminAuthMiddleware, requireMinRole(UserRole.USER), async (req: LocalAuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    // Single tenant: tenantId removido
...
/**
 * GET /api/secretarias/agricultura/propriedades/stats
 * Estatísticas das propriedades rurais
 */
router.get('/propriedades/stats', adminAuthMiddleware, requireMinRole(UserRole.USER), asyncHandler(async (req, res) => {
  // Estatísticas das propriedades
  const totalProperties = await prisma.ruralProperty.count({
    where: { status: 'ACTIVE' }
  });

  const totalAreaResult = await prisma.ruralProperty.aggregate({
    where: { status: 'ACTIVE' },
    _sum: { totalArea: true }
  });

  const cultivatedAreaResult = await prisma.ruralProperty.aggregate({
    where: { status: 'ACTIVE' },
    _sum: { cultivatedArea: true }
  });
  ...
  res.json({
    ...
    data: {
      ...
      totalArea: totalAreaResult._sum.totalArea || 0,
      cultivatedArea: cultivatedAreaResult._sum.cultivatedArea || 0
    }
  });
}));
 ...
 */
router.get('/monthly-stats', adminAuthMiddleware, requireMinRole(UserRole.USER), async (req: LocalAuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    // Single tenant: tenantId removido
 ...
 */
router.get('/dashboard', adminAuthMiddleware, requireMinRole(UserRole.USER), async (req: LocalAuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    // Single tenant: tenantId removido
