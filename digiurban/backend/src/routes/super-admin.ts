...
// ============================================================================
// 410 GONE — corte da dupla aceitação (Fases 1/6 do plano 2026-07-13)
// Endpoints de PLATAFORMA (tenants, invoices, leads, modules, platform-info,
// metrics, schema, migrations, backups) foram MOVIDOS para /api/platform,
// guardados por PlatformUser (middleware/platform-auth.ts). O role SUPER_ADMIN
// de tenant NÃO opera mais a plataforma (fechamento definitivo do achado R2).
// ============================================================================
const goneToPlatform = (_req: Request, res: Response) => {
  res.status(410).json({
    error: 'Endpoint movido',
    message: 'Este endpoint de plataforma foi movido para /api/platform e exige login de operador de plataforma.',
    code: 'PLATFORM_ENDPOINT_MOVED',
  });
};
// Express 5 (path-to-regexp v8): wildcard precisa ser nomeado (*splat)
router.all('/tenants', goneToPlatform);
router.all('/tenants/*splat', goneToPlatform);
router.all('/invoices', goneToPlatform);
router.all('/invoices/*splat', goneToPlatform);
router.all('/leads', goneToPlatform);
router.all('/leads/*splat', goneToPlatform);
router.all('/modules', goneToPlatform);
router.all('/platform-info', goneToPlatform);
router.all('/system/metrics', goneToPlatform);
router.all('/system/backup', goneToPlatform);
router.all('/system/backup/*splat', goneToPlatform);
router.all('/system/backups', goneToPlatform);
router.all('/schema', goneToPlatform);
router.all('/schema/*splat', goneToPlatform);
