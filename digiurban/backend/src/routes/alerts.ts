...
router.get('/:id/triggers', adminAuthMiddleware, async (req, res) => {
  try {
    const authReq = req as AuthenticatedRequest;
    if (!authReq.user) {
      return res.status(401).json({ success: false, error: 'Não autenticado' });
...
router.put('/triggers/:triggerId/resolve', adminAuthMiddleware, async (req, res) => {
  try {
    const authReq = req as AuthenticatedRequest;
    if (!authReq.user) {
      return res.status(401).json({ success: false, error: 'Não autenticado' });
...
router.post('/test/:id', adminAuthMiddleware, async (req, res) => {
  try {
    const authReq = req as AuthenticatedRequest;
    if (!authReq.user) {
      return res.status(401).json({ success: false, error: 'Não autenticado' });
