  ...
  try {
    ...
    if (service.formSchema && typeof service.formSchema === 'object' && 'properties' in service.formSchema) {
      ...
      if (service.formFieldsConfig && Array.isArray(service.formFieldsConfig)) {
        ...
        console.log('🔍 [DEBUG] formFieldsConfig encontrado, campos habilitados:', enabledFieldIds);
        ...
        console.log('🔍 [DEBUG] enabledFields encontrado:', enabledFieldIds);
      } else {
        console.log('⚠️ [DEBUG] Nenhuma configuração de campos encontrada para serviço:', service.id, 'mostrando todos os campos');
      }

      const fields = Object.entries(properties)
        .filter(([id]) => {
          // Se não tiver configuração de campos habilitados, mostrar todos
          if (!enabledFieldIds) return true;
          // Caso contrário, mostrar apenas os habilitados
