...
import { FaceMatchStatus } from '@prisma/client';
import { authenticateAdmin } from '../middleware/auth';
import facePlatformService from '../services/face-platform.service';
...
router.put('/configurations/:schoolId', async (req: Request, res: Response) => {
  try {
    const configuration = await facePlatformService.upsertSchoolConfiguration({
      ...req.body,
      unidadeEducacaoId: req.params.schoolId,
    });
...
router.get('/events', async (req: Request, res: Response) => {
  try {
    const events = await facePlatformService.listEvents({
      ...
      matchStatus: req.query.matchStatus as FaceMatchStatus | undefined,
