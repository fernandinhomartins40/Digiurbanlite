...
export function validateServiceFormData(
  ...
  if (formSchema.type === 'object' && formSchema.properties) {
    ...
    // ✅ COMPLETO: Campos do cidadão que devem ser ignorados na validação do customFormData
    // Suporta AMBOS os formatos: legacy (sem prefixo) e novo (com prefixo citizen_*)
    const citizenFieldsToIgnore = [
      ...
      // ✅ NOVO: Formato com prefixo citizen_* (normalizado)
    ...
    if (Array.isArray(cleanedSchema.required)) {
      // Filtrar required removendo campos citizen
      cleanedSchema.required = cleanedSchema.required.filter(
        (field: string) => !citizenFieldsToIgnore.includes(field.toLowerCase())
      );

      console.log('[Validation] Campos obrigatórios após filtrar citizen:', cleanedSchema.required);
    }

    // ✅ ADICIONAL: Remover campos citizen de properties também
    if (cleanedSchema.properties) {
      const originalPropsCount = Object.keys(cleanedSchema.properties).length;

      Object.keys(cleanedSchema.properties).forEach(key => {
        const lowerKey = key.toLowerCase();
        // Remover se começar com citizen_ OU se estiver na lista de campos citizen legacy
        if (lowerKey.startsWith('citizen_') || citizenFieldsToIgnore.includes(lowerKey)) {
          delete cleanedSchema.properties[key];
          console.log(`[Validation] Removido campo citizen de properties: ${key}`);
        }
      });

      const finalPropsCount = Object.keys(cleanedSchema.properties).length;
      if (originalPropsCount !== finalPropsCount) {
        console.log(`[Validation] Properties filtradas: ${originalPropsCount} → ${finalPropsCount}`);
      }
    }
...
function convertLegacyToJsonSchema(fields: any[]): any {
  ...
  fields.forEach(field => {
    ...
    // Esses campos são preenchidos pelo backend via citizenId autenticado
    // e não devem ser exigidos no customFormData do frontend
    if (id.toLowerCase().startsWith('citizen_')) {
      console.log(`[Validation] Ignorando campo citizen_* na validação: ${id}`);
