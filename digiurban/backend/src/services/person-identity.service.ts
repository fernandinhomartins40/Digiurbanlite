...
interface SyncUserIdentityInput extends PersonIdentityInput {
  userId: string;
}
...
export async function syncUserPersonIdentity(
  db: IdentityDbClient,
  input: SyncUserIdentityInput
) {
  const person = await ensurePersonRecord(db, input);

  const matchingCitizen = person.cpf
    ? await db.citizen.findFirst({
        where: { cpf: person.cpf },
        select: { id: true, personId: true },
      })
    : null;

  if (matchingCitizen && matchingCitizen.personId !== person.id) {
    await db.citizen.update({
      where: { id: matchingCitizen.id },
  ...
  if (input.cpf === null) {
    const linkedCitizenByPerson = await db.citizen.findFirst({
      where: { personId: person.id },
      select: { cpf: true },
    });

    if (!linkedCitizenByPerson?.cpf) {
  ...
  return { personId: person.id, linkedCitizenId: matchingCitizen?.id ?? null };
}

export async function syncCitizenPersonIdentity(
  db: IdentityDbClient,
  input: SyncCitizenIdentityInput
) {
  const person = await ensurePersonRecord(db, input);
