...
export function generateDefaultWorkflow(
  ...
  const stages: Omit<WorkflowStage, 'id'>[] = [
    ...
    },
    {
      ...
      requiredInputFieldIds: requiredFieldIds, // Campos obrigatórios do form
  ...
  if (otherDocs.length > 0) {
    stages.push({
      ...
      requiredInputFieldIds: allFieldIds,   // Todos os campos preenchidos
    ...
    stages.push({
      ...
      requiredInputFieldIds: allFieldIds,   // TODOS os campos
    ...
    stages.push({
      ...
      requiredInputFieldIds: allFieldIds,
      allowedActions: ['APPROVE', 'REJECT'],
      canSkip: false,
      requiresApproval: true
    });
...
export function generateSpecializedWorkflow(input: {
  ...
  if (analysis.hasIdentityDocuments || analysis.hasAddressDocuments) {
    ...
    stages.push({
      ...
      requiredInputFieldIds: [], // Não valida dados aqui, só documentos
      allowedActions: ['APPROVE', 'REJECT', 'CREATE_PENDING'],
      canSkip: false,
      requiresApproval: true,
      actionLabels: {
  ...
  if (formFields.length > 0) {
    ...
    stages.push({
      ...
      slaDays: dataSLA,
      requiredDocumentTypes: [],
      requiredInputFieldIds: requiredFieldIds, // TODOS os campos obrigatórios
      allowedActions: ['APPROVE', 'REJECT', 'CREATE_PENDING', 'REQUEST_INFO'],
      canSkip: false,
      requiresApproval: true,
      actionLabels: {
  ...
  if (analysis.hasSpecificDocuments || analysis.hasComplexFields) {
    ...
    stages.push({
      ...
      requiredInputFieldIds: allFieldIds, // Valida dados + docs específicos juntos
      allowedActions: ['APPROVE', 'REJECT', 'CREATE_PENDING', 'REQUEST_INFO'],
      canSkip: false,
      requiresApproval: true,
      actionLabels: {
    ...
    case 'CAPTURA_COMPLETA': // 🔵 Workflow COMPLETO (5-7 etapas)
      ...
      if (fields.length > 0) {
        stages.push({
          ...
          primaryTab: 'dados',
          requiredDocumentTypes: [],
          requiredInputFieldIds: requiredFieldIds,
          allowedActions: ['APPROVE', 'REJECT', 'CREATE_PENDING', 'REQUEST_INFO'],
          canSkip: false,
          requiresApproval: true,
          actionLabels: {
