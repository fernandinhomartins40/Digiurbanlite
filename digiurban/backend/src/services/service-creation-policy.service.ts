...
export function buildNoDataWorkflowTemplate(input: NoDataWorkflowInput): CreateWorkflowData | null {
  const totalSLA = Math.max(2, input.estimatedDays || 3);
  ...
  switch (input.subtype) {
    ...
    case 'EMISSAO_AUTOMATICA': {
      const stages = [
        ...
        buildStage({
          name: 'Emissão',
          description: 'Geração automática da informação ou documento',
          order: 2,
          slaDays: Math.max(1, totalSLA - 2),
          requiredDocumentTypes: [],
          requiredInputFieldIds: [],
          allowedActions: ['APPROVE'],
          canSkip: false,
          requiresApproval: false,
          actionLabels: { APPROVE: 'Emitir resposta' },
    ...
    case 'EMISSAO_ASSISTIDA': {
      ...
      const stages = [
        ...
        buildStage({
          name: 'Emissão',
          description: 'Emissão e assinatura do retorno',
          order: 3,
          slaDays: 1,
          requiredDocumentTypes: [],
          requiredInputFieldIds: [],
          ...
          canSkip: false,
          requiresApproval: false,
          actionLabels: { APPROVE: 'Emitir documento' },
          availableTabs: ['resumo', 'documentos-gerados', 'comunicacao'],
          primaryTab: 'documentos-gerados',
        }),
        buildStage({
    ...
    default: {
      const analysisSla = Math.max(1, totalSLA - 2);
      const stages = [
        buildStage({
          name: 'Recepção',
