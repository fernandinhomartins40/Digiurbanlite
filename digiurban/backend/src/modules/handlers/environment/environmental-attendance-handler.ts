...
export class EnvironmentalAttendanceHandler {
  static canHandle(moduleEntity: string): boolean {
    return moduleEntity === 'environmental_attendance' || moduleEntity === 'ATENDIMENTOS_MEIO_AMBIENTE';
  }
  ...
  static async execute(context: ModuleExecutionContext): Promise<ModuleExecutionResult> {
    ...
    return {
      ...
      entityType: 'EnvironmentalAttendance',
      data: attendance
    };
  }

  static async getById(id: string, prisma: PrismaClient) {
    return await prisma.environmentalAttendance.findUnique({
      where: { id }
    });
  }
  ...
  static async updateStatus(id: string, status: string, notes: string | undefined, prisma: PrismaClient) {
    return await prisma.environmentalAttendance.update({
      where: { id },
      data: {
        status,
