...
RUN set -e && \
    ...
    test -d ".next" || { echo "❌ ERRO: .next não existe!"; exit 1; } && \
    test -f ".next/BUILD_ID" || { echo "❌ ERRO: BUILD_ID não gerado!"; exit 1; } && \
    test -d "node_modules" || { echo "❌ ERRO: node_modules não existe!"; exit 1; } && \
