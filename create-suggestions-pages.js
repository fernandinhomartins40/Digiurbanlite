...
const departments = [
  { slug: 'administracao', name: 'Administração', color: 'blue' },
  { slug: 'defesa-civil', name: 'Defesa Civil', color: 'orange' },
  { slug: 'desenvolvimento-economico', name: 'Desenvolvimento Econômico', color: 'green' },
  { slug: 'financas', name: 'Finanças', color: 'emerald' },
  { slug: 'mobilidade-urbana', name: 'Mobilidade Urbana', color: 'cyan' },
  { slug: 'politicas-mulheres', name: 'Políticas para Mulheres', color: 'pink' },
  { slug: 'tecnologia-inovacao', name: 'Tecnologia e Inovação', color: 'purple' },
  { slug: 'transportes-transito', name: 'Transportes e Trânsito', color: 'yellow' }
];

const template = (dept) => `'use client';
...
export default function ${dept.name.replace(/\s+/g, '')}SugestoesPage() {
  const router = useRouter();
  const { suggestions, isLoading } = useServiceSuggestions('${dept.slug}');
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      ...
      <div className="mb-8">
        <Button
          variant="ghost"
          onClick={() => router.push('/admin/secretarias/${dept.slug}')}
          className="mb-4"
        >
          <ArrowLeft className="h-4 w-4 mr-2" />
          Voltar para ${dept.name}
        </Button>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-2">
              Explore todas as {suggestions.length} sugestões disponíveis para ${dept.name}
            </p>
          </div>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredSuggestions.map((suggestion) => (
            <Card key={suggestion.id} className="border-${dept.color}-200 bg-${dept.color}-50/50 hover:shadow-lg transition-shadow">
              <CardHeader>
                <CardTitle className="text-lg flex items-center gap-2">
                  <FileCheck className="h-5 w-5 text-${dept.color}-600" />
                  {suggestion.name}
                </CardTitle>
...
departments.forEach(dept => {
  const dirPath = path.join('c:\\Projetos Cursor\\Digiurbanlite\\digiurban\\frontend\\app\\admin\\secretarias', dept.slug, 'sugestoes');
  ...
  // Escrever arquivo
  fs.writeFileSync(filePath, template(dept));
  console.log(`✅ Criado: ${filePath}`);
});

console.log('\n🎉 Todas as páginas de sugestões foram criadas!');
